...
pub fn thread_generate_prime(found_total: &AtomicUsize, n: usize) -> Option<BigInt<{2 * KEY_SIZE}>> {
...
pub fn generate_primes(n: usize) -> Vec<BigInt<{2 * KEY_SIZE}>> {
    let cores = std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1);
    let (tx, rx) = mpsc::channel::<BigInt<{2 * KEY_SIZE}>>();
...
Also add Montgomery reduction to BigIntMod
